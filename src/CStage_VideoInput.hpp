 ...
 */
class CStage_VideoInput: public CPipelineStage, ImageReader {
	/**
	 * global parameters
	 */
...
public:
	...
	CStage_VideoInput(CParameters &i_cParameters) :
			CPipelineStage("ImageInput"), ImageReader(
					i_cParameters.video_device, IO_METHOD_MMAP,	// io method
					1,	// force format
					0	// frame number
					), cParameters(i_cParameters) {
		setVideoInputSize(cParameters.input_video_width,
				cParameters.input_video_height);

		std::cout << "V4L: Opening video device " << i_cParameters.video_device
				<< std::endl;
		open_device();
		...
		if (cParameters.verbosity_level > 5)
			std::cout << "Using input video size of " << image_width << " x "
...
private:
	 ...
	 */
	void processImage(const void* p, int i_size) {
