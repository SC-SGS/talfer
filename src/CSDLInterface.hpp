...
#if defined(__APPLE__)
		// Enabled swap synchronization with vertical retrace
		GLint swap = 1;
...
public:
	CSDLInterface() :
			bpp(0), mouse_x(-1), mouse_y(-1), prev_fps_update_ticks(0), buffer_swaps_since_last_fps_update(
					0), mouse_buttons(0) {
		setup();
	}
	...
	virtual void sdl_handle_key_down(SDL_keysym &i_keysym) = 0;

	virtual void sdl_handle_key_up(SDL_keysym &i_keysym) = 0;

	virtual void sdl_handle_mouse_button_down(int i_button_id,
			int i_mouse_buttons, int mouse_x, int mouse_y) = 0;

	virtual void sdl_handle_mouse_button_up(int i_button_id,
			int i_mouse_buttons, int mouse_x, int mouse_y) = 0;

	virtual void sdl_handle_mouse_motion(int i_mouse_buttons, int mouse_x,
			int mouse_y) = 0;
	...
	void sdl_process_events() {
		...
		while (SDL_PollEvent(&event)) {
			...
			case SDL_KEYUP:
				/* Handle key releases. */
				sdl_handle_key_up(event.key.keysym);
				break;
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				sdl_handle_mouse_button_down(event.button.button, mouse_buttons,
						mouse_x, mouse_y);
				break;

			case SDL_MOUSEBUTTONUP:
				mouse_buttons &= ~(1 << event.button.button);
				sdl_handle_mouse_button_up(event.button.button, mouse_buttons,
						mouse_x, mouse_y);
				break;
	...
	void sdl_swap_buffers() {
		...
		if (delta_ticks >= 1000) {
			double fps = (double) buffer_swaps_since_last_fps_update
					/ ((double) delta_ticks * 0.001);
			char buffer[1024];
