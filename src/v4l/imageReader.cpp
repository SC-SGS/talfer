...
ImageReader::ImageReader() :
		dev_name("/dev/video0"), io(IO_METHOD_MMAP), fd(-1), frame_number(70), force_format(
				1), frame_ctr(0), image_width(-1), image_height(-1) {
	;
}

ImageReader::ImageReader(std::string &dev_name) :
		dev_name(dev_name), io(IO_METHOD_MMAP), fd(-1), frame_number(70), force_format(
				1), frame_ctr(0), image_width(-1), image_height(-1) {
	;
}

ImageReader::ImageReader(std::string &dev_name, io_method io, int force_format,
		int frame_number) :
		dev_name(dev_name), io(io), frame_number(frame_number), force_format(
				force_format), image_width(-1), image_height(-1) {
	;
}
...
void ImageReader::open_device(void) {
	...
	if (-1 == stat(dev_name.c_str(), &st)) {
		std::cerr << "Cannot identify '" << dev_name << "': " << errno
	...
	this->fd = v4l2_open(dev_name.c_str(), O_RDWR /* required */| O_NONBLOCK,
			0);

	if (-1 == fd) {
		std::cerr << "Cannot open '" << dev_name << "': " << errno
				<< strerror(errno) << std::endl;
		exit (EXIT_FAILURE);
	}
}
